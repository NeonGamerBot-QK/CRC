...
if(!config.emailConfig.username) return res.status(500).json({ status: 500, message: "Email not configured" })
if(!config.emailConfig.password) return res.status(500).json({ status: 500, message: "Email not configured" })
if(!config.emailConfig.host) return res.status(500).json({ status: 500, message: "Email not configured" })
if(!config.emailConfig.port) return res.status(500).json({ status: 500, message: "Email not configured" })
