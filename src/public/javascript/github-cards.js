fetch("https://api.github.com/users/NeonGamerBot-QK/repos?per_page=100")
  .then((r) => r.json())
  .then((d) => {
    let cardsDiv = document.getElementById("cards");
    const length = d.filter(
      (e) => !e.fork && !e.archived && e.description,
    ).length;
    cardsDiv.innerHTML = "";
    cardsDiv.className =
      "grid grid-cols-2  lg:grid-cols-3 p-5 mt-2 p-2 border-20 rounded-lg shadow-xl";
    ...
    d.filter((e) => !e.fork && !e.archived && e.description).forEach((repo) => {
      ...
      //   </div>`
      card.className =
        "card  bg-base-100 shadow-xl lg:m-10 md:m-5 m-2 transition duration-500 ease-in-out transform hover:-translate-y-1 lg:hover:scale-110 hover:scale-105";
